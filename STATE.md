...
Two instrument defects fixed: a generation reads as `budget` only when it was
cut off *and* this call's deadline is what cut it (the request ceiling is
`min(config.timeoutMs, remaining)`, so either signal alone mislabels in its own
direction), and the per-round trace reaches telemetry — without it B7 was never
measurable at all.
