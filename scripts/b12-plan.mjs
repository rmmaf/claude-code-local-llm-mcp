...
export function buildPlan() {
  ...
  return {
    ...
    _typesOnlyEscapeRoute: [
      "A SECOND THING THIS DESIGN CANNOT RULE OUT, named here rather than rediscovered later",
      "as a defect in one base. `_multiCellGap` is the precedent for the shape of this note.",
      "",
      "THIRTY TASKS ARE SCORED BY tsc ALONE, and acceptance is the predicate's exit code and",
      "nothing else. Seven of their sites drop a `| null` from a signature — tylmsopt,",
      "typarseblk, tycmdblock, tysharedres, tystatusout, tycorpusrow and tymemrec. For every",
      "one of them there are TWO ways to make tsc exit 0: restore the annotation, or satisfy",
      "the narrowed type by changing what the function DOES.",
      "",
      "MEASURED on tymemrec, built and run rather than argued: replacing its three `return",
      "null` statements with os.totalmem()/os.freemem() fallbacks is three lines, stays inside",
      "fileScope, makes tsc exit 0, and leaves the function reporting fabricated numbers where",
      "the parent reported 'no reading'. tests/memory.test.ts catches it; the predicate never",
      "runs that suite. So the session scores FIXED.",
      "",
      "AN EARLIER ADJUDICATION GOT THE RIGHT ANSWER BY THE WRONG ROUTE, and the route matters",
      "because it is what a reader will reuse. It passed tymemrec on the grounds that an",
      "already-published sibling has the same shape — an anecdote. The property is countable,",
      "and counted it is 7 of 30, which is why rejecting this base would mean retiring six",
      "published ones.",
      "",
      "WHAT IS STILL OPEN, stated so nobody reads this as closed: the seven deal 4 into A and",
      "3 into B, which is BALANCED ACROSS MANIFESTS and is NOT the same as unbiased between",
      "arms. A and B are two sealed RUNS; treatment and control are per-observation through",
      "abPairs, which manifest-config.json NOW DECLARES — and the answer is worse than the",
      "question expected. An earlier version of this note said the seven would be noise if each",
      "appeared in both arms of its pair. That is refuted: pairing makes the escape route equally",
      "AVAILABLE to both arms, but availability is not probability. Only the treatment arm has",
      "`repair`, which loops until its checks pass, and on a tsc-only task that loop has no signal",
      "separating a restored annotation from a behaviour-changing patch (src/tools/repair.ts:628).",
      "So arm-dependent route choice survives as a possible TREATMENT EFFECT, not as noise.",
      "",
      "AND ONLY TWO OF THE SEVEN ARE PAIRED, both in manifest B: tylmsopt and typarseblk sit in",
      "B's first six, and manifest A's first six contain none. The pairs are the first six ids of",
      "each committed order, a rule with no clause to tune, so which escape-route tasks fall",
      "inside is not a choice. The other five are observed in one arm only.",
      "",
      "THE ONLY LEVER IS THE PROSE. `symptom` is what tells a session whether it is looking at",
      "a type that lost its nullable half or at a function that should stop returning null,",
      "and nothing compares that prose to anything. The seven were authored before this was",
      "understood; they are deliberately NOT being edited to steer, because a thumb on the",
      "scale for some tasks and not others is the operator confound the preregistration's",
      "CHANNEL 1 already names.",
    ],
    specRoot: "b12-corpus",
